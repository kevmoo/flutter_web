...
abstract class TestWidgetsFlutterBinding extends BindingBase
  ...
  void initInstances() {
    timeDilation =
        1.0; // just in case the developer has artificially changed it for development
  ...
  Future<void> _runTest(
    ...
    final VoidCallback testCompletionHandler =
  ...
  Future<void> _runTestBody(
      Future<void> testBody(), VoidCallback invariantTester) async {
    assert(inTest);
...
class AutomatedTestWidgetsFlutterBinding extends TestWidgetsFlutterBinding {
  ...
  Future<void> pump(
    ...
    return TestAsyncUtils.guard<void>(() {
      assert(inTest);
      assert(_clock != null);
      if (duration != null) _currentFakeAsync.elapse(duration);
      _phase = newPhase;
  ...
  Future<T> runAsync<T>(
    ...
    assert(() {
      if (_pendingAsyncTasks == null) return true;
      throw test_package.TestFailure('Reentrant call to runAsync() denied.\n'
  ...
  void _verifyInvariants() {
    ...
    assert(_currentFakeAsync.periodicTimerCount == 0,
        'A periodic Timer is still running even after the widget tree was disposed.');
    assert(_currentFakeAsync.nonPeriodicTimerCount == 0,
...
//    assert(!autoUpdateGoldenFiles);
  ...
  void handleDrawFrame() {
    assert(_doDrawThisFrame != null);
    if (_doDrawThisFrame) super.handleDrawFrame();
    ...
    switch (source) {
      case TestBindingEventSource.test:
        if (!renderView._pointers.containsKey(event.pointer)) {
          assert(event.down);
          renderView._pointers[event.pointer] =
...
class _LiveTestRenderView extends RenderView {
  ...
  @override
  set configuration(covariant TestViewConfiguration value) {
    super.configuration = value;
  }

  final VoidCallback onNeedPaint;

  final Map<int, _LiveTestPointerRecord> _pointers =
      <int, _LiveTestPointerRecord>{};
  ...
  void _setDescription(String value) {
    ...
    _label.layout();
    if (onNeedPaint != null) onNeedPaint();
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (_pointers.isNotEmpty) {
      ...
      if (dirty && onNeedPaint != null) scheduleMicrotask(onNeedPaint);
    }
